...
def fetch_company_info(ticker):
    try:
        ...
        return {"name": info.get("longName", ticker), "sector": info.get("sector", ""), "industry": info.get("industry", ""), "marketCap": info.get("marketCap", 0), "quoteType": info.get("quoteType", "")}
    except:
        return {}
...
def is_news_relevant_ai(ticker, company_name, title, description=""):
    ...
    try:
        ...
        rsp = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are a news relevance filter. Return only RELEVANT or NOT_RELEVANT. Filter out false positives like BB gun for BlackBerry stock."},
...
def fetch_news_from_newsapi(ticker, company_name="", is_korean=False):
    """NewsAPI를 사용해 뉴스를 가져오고 AI 필터링을 적용합니다."""
    if not NEWS_API_KEY:
        return []
    try:
        ...
        params = {"q": query, "sortBy": "publishedAt", "language": lang, "pageSize": 30, "apiKey": NEWS_API_KEY}
        ...
        print(f"NewsAPI 오류 ({ticker}): {e}")
        return []

def fetch_news(ticker, company_name=""):
    """종목 뉴스를 가져옵니다. NewsAPI + AI 필터링."""
    try:
        ...
        try:
            ...
            if ticker == "^KS11":
                ...
                sq = "Nasdaq Composite index"
            else:
                sq = f'"{company_name}" OR "{clean_ticker}"' if company_name else f'"{clean_ticker}"'
            s = gn.search(sq)
            items = []
            ...
            print(f"Google News 오류 ({ticker}): {e}")
            return []
    except Exception as e:
